...
export function validateWorkflowName(input: string | undefined): WorkflowName | undefined {
	...
	if (!normalized) {
		throw new ValidationError(
			`Unknown workflow '${input}'. Valid values: delivery, co-creation (aliases: ov-delivery, ov-co-creation, co_creation, cocreation)`,
			{
				field: "workflow",
				value: input,
			},
...
export function repoRootFromCommandDir(commandDir: string): string {
	// Command modules live at <package-root>/src/commands/ — resolve two levels up.
